...
- **字表主題包(THEME PACKS)**(2026-07-11 使用者確認想做,後續排程):
  天氣、動物、食物、程式術語等可切換字表,在選單或 OPTIONS 選擇。
  資料層現成:`words.ts` 已按 short/medium/long 分級,新增主題 = 多一組 POOLS + 選擇 UI;
  每包需維持「各級 100+、字首分散、全小寫 a–z」三個品質標準(驗證腳本模式見 phase 16)。
  可考慮排行榜是否分主題(建議不分,主題只影響字面不影響難度)。
- 行動裝置輸入(虛擬鍵盤 / 觸控)
- 單字母模式(初學者用)
